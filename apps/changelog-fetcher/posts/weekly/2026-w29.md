...
今回選んだ8件を見ると、サブエージェントの隔離やプランモード、パーミッションルールに関する修正が並びました。自分が実際に踏んだ挙動が直っているものもいくつか混ざっています。
...
タイムアウトやトークン予算を `64_000` や `1e6` の形で書けるようになるのは、地味ですが読み間違いが減ってありがたい変更です。ゼロを数え直していた手間がなくなるだけで、設定ファイルを開いたときの気持ちが少し軽くなります。
...
プランに切り替えた直後は問題なくても、しばらくやり取りを重ねた後で `rm` が走ったことは自分でもありました。しくみ側で塞がれるのは助かるのですが、プラン中は計画に専念してもらうよう指示側の工夫も続けたいです。
...
組織単位で利用ログを集めているとき、ツール呼び出しの出所まで辿れるようになるのは地味に効きます。振り返りの場で「この呼び出しはどのワークフロー由来か」と聞かれて答えられずにいたので、その手のもどかしさから解放されそうです。
...
自分は踏んでいないのですが、開発速度を考えるとこの手の回帰は避けきれないのだろうと想像します。原因まで開示したうえで直しに来る姿勢は、任せる側として素直に信頼できます。
