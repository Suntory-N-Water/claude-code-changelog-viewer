...
| [Hook restrictions](/docs/en/settings#hook-configuration) | Restrict which hooks run, narrow `statusLine`, `fileSuggestion`, and `subagentStatusLine` to managed settings, and restrict HTTP hook URLs | `allowManagedHooksOnly`, `allowedHttpHookUrls` |
