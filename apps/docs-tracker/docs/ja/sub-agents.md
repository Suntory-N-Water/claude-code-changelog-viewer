...
`name` 値をツリー全体で一意に保ちます：1 つのスコープ内の 2 つのファイルが同じ名前を宣言する場合、Claude Code は 1 つのみを読み込みます。v2.1.196 以降、`/doctor` を実行すると、同じスコープの重複するエージェント名が報告され、どの定義がアクティブであるかが表示されます。
